...
module.exports =
    class collectionFilter {
        constructor(collection, filterParams) {
            ...
            Object.keys(filterParams).forEach(function (paramName) {
                ...
                switch (paramName) {
                    ...
                    case "limit": this.limit = parseInt(paramValue); break;
                    case "offset": this.offset = parseInt(paramValue); break;
        ...
        limitFunc() {
            let min = this.limit * this.offset
            let max = this.limit * (this.offset + 1);
            let cloneFilteredCollection = [];

            if(max > this.filteredCollection.length)
                max = this.filteredCollection.length;
            for (let index = min; index < max; index++) {
                cloneFilteredCollection.push(this.filteredCollection[index]);
            }
            this.filteredCollection = cloneFilteredCollection;
        }
